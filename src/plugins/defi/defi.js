...
export class DeFi {
    ...
    _setTokens(_tokens) {
        ...
        this.deamToken = _tokens.find((_item) => _item.symbol === 'DEAM');
    ...
    */
    ...
    canTokenBeDeposited(_token) {
        return _token && _token.isActive && _token.canDeposit && _token.symbol !== 'DEAM';
    }
    ...
    canTokenBeTraded(_token) {
        ...
        return _token && _token.isActive && (_token.canTrade || _token.symbol === 'DEAM');
     ...
     * @return {Promise<Number>}
     */
    async fetchDEAMTokenPrice(_to = 'USD') {
        const data = await this.apolloClient.query({
            query: gql`
